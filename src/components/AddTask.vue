...
async function addTask() {
  ...
  axios.post(`${API_URL}/Tasks/`, data).then((response) => {
...
<template>
  <div class="border-4 border-blue-400 rounded-lg overflow-x-auto inline-block p-5">
    <form @submit.prevent="addTask">
      ...
      <button
        @click="addTask"
        class="items-center py-2 px-4 text-white font-bold bg-blue-400 hover:bg-blue-500 text-lg rounded-lg mt-6 shadow-lg w-full"
      >
        Add Task
      </button>
    </form>
